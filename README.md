...
- **레이아웃 최종 수정:** 반복적으로 발생하던 '모든 화면 동시 표시' 오류를 해결하기 위해, 앱의 최상위 HTML 구조를 재정의하여 레이아웃 안정성을 근본적으로 확보했습니다.
- **동기화 로직 버그 수정:** 오프라인에서 생성된 신규 회원이 서버에 제대로 동기화되지 않던 버그를 수정하여 데이터 정합성을 높였습니다.
- **Firebase 초기화 오류 수정:** 회원 등록 등 특정 작업 시 Firebase 앱이 중복으로 생성되어 오류가 발생하던 문제를 해결했습니다.
