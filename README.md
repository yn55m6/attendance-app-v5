...
- **리포트 그래프 렌더링 버그 수정:** '신규 회원' 그래프가 보이지 않던 문제를 해결했습니다. 이제 각 리포트 탭은 활성화될 때만 해당 콘텐츠를 렌더링하여, 그래프가 항상 올바르게 표시됩니다.
- **리포트 그래프 시각 효과 개선:** '신규 회원' 리포트 그래프의 숫자가 더 잘 보이도록 색상과 크기를 조정하고, 그래프가 천천히 올라오는 애니메이션 효과를 추가했습니다.
- **UI 안정성 강화:** 테마 색상이 적용되지 않아 UI가 보이지 않던 문제를 근본적으로 해결하기 위해 모든 테마 관련 스타일에 기본 색상(Fallback)을 추가했습니다.
