...
- **로컬 우선 아키텍처:** 기존 **Firebase(클라우드 DB)** 의존성을 제거하고, 브라우저 내장 데이터베이스인 **IndexedDB**를 사용하도록 전환했습니다. 이제 인터넷 연결 없이도 모든 기능을 오프라인으로 사용할 수 있으며, 별도 서버 설정이 전혀 필요 없습니다.
- **데이터 유실 주의:** 모든 데이터는 이제 사용자 **브라우저 내에만 저장**됩니다. 브라우저의 캐시나 데이터를 삭제할 경우 모든 출석 기록과 회원 명단이 영구적으로 삭제되니 주의가 필요합니다.
...
```mermaid
graph TD
    A[QR 코드 스캔] --> B{"유효성 검사<br/>(요일/시간)"}
    B -- 실패 --> C[에러 알림 및 URL 초기화]
    B -- 성공 --> D[회원 출석 화면으로 이동<br/>(명단 그리드 표시)]
    D --> E{회원 선택}
    E -- 명단에서 선택 --> F[회원 선택]
    E -- 검색 후 신규 등록 --> G[신규 회원 자동생성]
    F --> H{중복 출석 확인}
    G --> H
    H -- 중복 --> I[알림 표시]
    H -- 통과 --> J["IndexedDB 저장<br/>(logs 저장소)"]
    J --> K[완료 알림 (실행 취소 가능)]
```
