...
- **레이아웃 오류 근본 해결 (구조 변경):** 반복되던 '모든 화면 동시 표시' 오류를 해결하기 위해, 앱의 초기화 로직을 전면 재설계했습니다. 이제 로그인 화면을 먼저 안전하게 표시한 후 데이터를 불러오도록 변경하여, 로딩 순서나 캐시 문제로 인해 레이아웃이 깨지는 현상을 원천적으로 차단했습니다. (※문제 해결을 위해 서비스 워커 기능은 임시 비활성화되었습니다.)
